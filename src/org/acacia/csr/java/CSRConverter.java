...
/**
 * Converts and edgelist to CSR format. Note that in an edge list we know by
 * default that all the vertices are connected with each other. Therefore no
 * need of worrying whether to leave empty lines for unconnected vertices.
 * However, we can see that certain CSR files leave empty lines for their
 * corresponding unconnected vertices.
 * 
 * @author miyuru
 *
 */
public class CSRConverter {
    public static class InvertedMapper extends MapReduceBase
            implements
        ...
        private void connectToVertexCounterService(String contactHost) {
            try {
                Socket socket = new Socket(contactHost,
        ...
        public void map(LongWritable key, Text value,
            ...
            while (itr.hasMoreTokens()) {
                ...
                if (Character.isDigit(stVert.charAt(0))
                        && Character.isDigit(edVert.charAt(0))) {

                    if (zeroFlag) {
                        startVertex = new Text(""
                                + (Integer.parseInt(stVert.toString()) + 1));
                        endVertex = new Text(""
                                + (Integer.parseInt(edVert.toString()) + 1));
                    } else {
                        startVertex = new Text(""
                                + (Integer.parseInt(stVert.toString())));
                        endVertex = new Text(""
                                + (Integer.parseInt(edVert.toString())));
                    }
                    ...
                    if (zeroFlag) {
                        lStartVertex
                                .set(Integer.parseInt(stVert.toString()) + 1);
                    } else {
                    ...
                    // CSR must output undirected graphs. Therefore we create
                    // two dirceted edges for each edge found.
    ...
    public static class InvertedReducer extends MapReduceBase implements
            org.apache.hadoop.mapred.Reducer<LongWritable, Text, Text, Text> {
    ...
    private static void printUsage() {
        ...
        System.out
                .println(" hadoop jar acacia.jar org.acacia.csr.java.CSRConverter <inputfile-path-on-hdfs>");
        System.out.println("----------------------------");
    }
