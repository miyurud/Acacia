...
public class VertexPartitioner implements Partitioner<LongWritable, Text> {
    ...
    // @Override
    // Here we need to identify the vertex count/largest veretx. Then we can
    // decide what number of vertices will be in a partition. Then what we need
    // to do is to
    // do a for loop and increment a counter until the key is in between the two
    // incremented values. Once we find that condition satisfied, we return
