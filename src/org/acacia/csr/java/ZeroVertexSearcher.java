...
public class ZeroVertexSearcher {
    ...
    public static class TokenizerMapper extends
        ...
        public void map(Object key, Text value, Context context)
            ...
            while (itr.hasMoreTokens()) {
                if (itr.nextToken().equals("0")) {
                    FileSystem fs = FileSystem.get(new JobConf());
                    FSDataOutputStream out = fs.create(new Path(
                            "/user/miyuru/zout/zout"));
                    out.writeUTF("1");
                    out.close();

                    throw new ZeroFoundException(
                            "Vertex 0 was found in Task : "
    ...
    public static void main(String[] args) throws Exception {
        /*
         * String dir1 = "/user/miyuru/wcout"; //We first delete the temporary
         * directories if they exist on the HDFS FileSystem fs1 =
         * FileSystem.get(new JobConf());
         * 
         * if(fs1.exists(new Path(dir1))){ fs1.delete(new Path(dir1), true); }
         * 
         * JobConf conf = new JobConf(); conf.setNumMapTasks(96); Job job = new
         * Job(conf, "word count"); job.setJarByClass(WordCount.class);
        ...
        conf3.set("mapred.map.max.attempts", "0");// If the job fails we assume
                                                  // that it happens because we
                                                  // found zero. Therfore we do
                                                  // not attempt again.
