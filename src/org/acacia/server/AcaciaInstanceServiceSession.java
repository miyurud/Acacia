...
public class AcaciaInstanceServiceSession extends Thread {
	...
	// private GraphDatabaseService graphDB;//This is a reference to the
	// original DB
	...
	// Note : Feb 4 2015 - Since we need to deal with the
	// <praphID>_<partitionID> scenario,
	 ...
	 * @param socket
	 */
	public AcaciaInstanceServiceSession(Socket socket,
			HashMap<String, AcaciaHashMapLocalStore> db, ArrayList<String> grp) {
		sessionSkt = socket;
		graphDBMap = db;
		loadedGraphs = grp;
		dataFolder = Utils_Java
				.getAcaciaProperty("org.acacia.server.instance.datafolder");
		System.out.println("MMMMMMMMMMMMM");
	}

	public void setGraphDBMap(HashMap<String, AcaciaHashMapLocalStore> db,
	...
	public void run() {
		...
		// First we need to check if the directory exist. If not we need to
		// create it.
		...
		try {
			...
			while ((msg = buff.readLine()) != null) {
				...
				// +Miyuru (May 2014) - This seems rather long if else
				// structure. Better have some other mechanism to
				// query parsing if possible.
				if (msg.equals(AcaciaInstanceProtocol.HANDSHAKE)) {
					out.println(AcaciaInstanceProtocol.HANDSHAKE_OK);
					out.flush();
					// Here we get the host name of the main server. We need
					...
					sessionSkt.close();
					fireShutdownEvent(new ShutdownEvent(
							"Got shutdown request from host : " + msg));
					break;
					...
					// out.flush();
					//
					// //From here onwards we should receive a collection of
					// edges
					// msg = buff.readLine();
					//
					// String graphID = msg;
					...
					// String[] vertsArr = msg.split(" ");//We expect the edge
					// to be split by a space.
					// try{
					// insertEdgeUsingIndex(Long.parseLong(vertsArr[0]),
					// Long.parseLong(vertsArr[1]));
					// }catch(NumberFormatException ex){
					// //Just ignore. Expect the messages only in the for <long>
					// <long> <long>
					...
					fireDBTruncateEvent(new DBTruncateEvent(
							"Truncating Acacia Instance"));
					...
					Logger_Java.info("Counting vertices, graph id is : "
							+ graphID);
					...
					// Now we want to get the exact partition id that we will be
					// deleting.
					// Because there will be other places running in this node
					// that will handle other
					...
					System.out.println("Batch-------------------------------------up------------------------------load");
					out.println(AcaciaInstanceProtocol.OK);
					out.flush();
					...
					// the fileName contains the graph partitionID. So we do not
					// have to specifiy that again here.
					...
					String hostList = buff.readLine().trim(); // Here we get the
																// list of hosts
																// involved in
																// the Top-K
																// PageRank
					...
					msg = buff.readLine().trim(); // Here we get the K value of
													// Top-K PageRank

					msg = pageRankTopKLocal(graphID, partitionID, hostList,
					...
					// Once we get the graph ID we need to figureout on which
					// partition are we going to operate on
					...
					String partitionID = msg;
					System.out
							.println("============= MMMMMMMMMMMMMMMMMMMMKKKKKKKKKKKKKKK =====================");
					...
					System.out.println("Wade hariiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii");
					while (!isUploadCompleted(fullFilePath)) {
						msg = buff.readLine().trim();
	...
	private String countTrainglesLocal(String g, String p) {
		...
		if (defaultGraph == null) {
			...
			if (graphDB == null) {
				...
				System.out
						.println("IIIIIIIIAAAAAAAMMM Now here : isGraphDBExists()");
	...
	private String pageRankTopKLocal(String graphID, String partitionID,
			String hostList, int k) {
		String result = null;
		AcaciaHashMapLocalStore graphDB = null;
		...
		result = ApproxiRank.run(graphID, partitionID, graphDB, hostList,
				serverHostName, k);
	...
	private String getInstanceStatus() {
		...
		try {
			...
			BufferedReader reader = new BufferedReader(new FileReader(
					"conf/localstorage.txt"));

			while ((line = reader.readLine()) != null) {
				// Here we just read first line and then break. But this may not
				// be the best option. Better iterate through all the
			...
			Logger_Java
					.error("Connecting to localhost got error message (11) : "
							+ e.getMessage());
		...
		// First we need to check whether the Acacia's data directory exists or
		// not.
		...
		if (fileChk.exists() && fileChk.isDirectory()) {
			...
			try {
				...
				Logger_Java
						.error("Creating the Acacia data folder threw an Exception : "
								+ ex.getMessage());
			}
		}

		long size = Math.round(FileUtils.sizeOfDirectory(new File(dataFolder))
				/ (1024 * 1024)); // Divide by 1024*1024 to convert to MB
		double d = size / Double.parseDouble(instanceCapacity);
		java.text.DecimalFormat f = new java.text.DecimalFormat();
		f.setMaximumFractionDigits(2);

		// The status message is of the format <Size of the local storage
		// usage>:<Max local storage size>:<percentage of local storage used>
	...
	/**
	 * Batch upload process will be completed by deleting the transfered file.
	 * Therefore we check the status of the file and report the status.
	 * 
	 * @param filePath
	 * @return
	 */
	...
	private boolean isGraphDBExists(String graphID, String partitionID) {
		...
		if (f.isDirectory()) {
			System.out
					.println("*********************>>>The graph db directory exists ****************");
			result = true;
		} else {
			System.out
					.println("*********************>>>The graph db directory does not exist ****************");
			result = false;
		}
	...
	public void unzipAndBatchUpload(final String graphID, final String partitionID) {
		...
		try {
			...
			// Process p = r.exec("mv /tmp/dgr/file" + fileName +
			// " /tmp/dgr/file" + fileName + ".gz");
			// p.waitFor();
			//
			...
			BufferedReader br;
			br = new BufferedReader(new FileReader("/tmp/dgr/" + graphID + "_"
					+ partitionID), 10 * 1024 * 1024);
			line = br.readLine();
			...
			// Also we need to add a catalog record in the instance's local data
			// store about the graph and its partition IDs
	...
	private void unzipAndBatchUploadCentralStore(String graphID, String partitionID) {
		try {
			AcaciaHashMapCentralStore centralStore = new AcaciaHashMapCentralStore(Integer.parseInt(graphID), Integer.parseInt(partitionID));
			centralStore.loadGraph();
			Runtime r = Runtime.getRuntime();
			Process p = r.exec("unzip /tmp/dgr/" + graphID + "_" + partitionID + ".zip -d /tmp/dgr/");
			p.waitFor();
...
//			Splitter splitter = null;
//			BufferedReader br;
//			br = new BufferedReader(new FileReader("/tmp/dgr/" + graphID + "_" + partitionID + ".log"), 10 * 1024 * 1024);
//			line = br.readLine();
//			while(line != null){
//				if(line.contains("")){
//					
...
//			}
	...
	private void writeCatalogRecord(String record) {
		...
		try {
			...
			BufferedWriter writer = new BufferedWriter(new FileWriter(catalog,
					true));// We are appending to the file rather than replacing
			writer.write(record);
			writer.write("\n");// We need a new line to separate between two
								// records.
			writer.flush();
			writer.close();
		} catch (IOException e) {
			System.out
					.println("There is an error is writing to the AcaciaInstance's catalog.");
		}
	}

	/**
	 * We are deleting all the content corresponding to this subgraph. Note that
	 * it should be subgraph not the entire graph. Because there may be multiple
	 * places handling multiple subgraphs of the same graph located on a
	 * particular node. Therefore, every place should delete only the subgraph
	 * that it is responsible of.
	 * 
	 * This pattern of operation is required only when dealing with the graph
	 * data.
	 * 
	 * @param graphID
	 * @return
	 */
	// ToDO: In future we need to update the catalog file located in the data
	// directory.
	private String deleteGraph(String graphID, String partitionID) {
		...
		if (graphDBMap.containsKey(graphID)) {
			Logger_Java.info("Unloading the graph - " + graphID + ":"
					+ partitionID);
			unloadLocalStore(graphID, partitionID);
		} else {
			Logger_Java
					.info("The following graph was not loaded to the system - "
							+ graphID + ":" + partitionID);
		}

		// Next we need to delete the file content from the acacia instance
		// directory.
		try {
			Logger_Java.info("Deleting the folder: " + dataFolder + "/"
					+ graphID + "_" + partitionID);
			FileUtils.deleteDirectory(new File(dataFolder + "/" + graphID + "_"
					+ partitionID));
			Logger_Java.info("Done deleting.");
		} catch (IOException ec) {
			Logger_Java
					.error("Error in deleting the file : " + ec.getMessage());
		}

		/*
		 * //Here we may have more than one file to handle. File folder = new
		 * File(dataFolder); File[] files = folder.listFiles(new
		 * FilenameFilter(){
		 * 
		 * public boolean accept(File dir, String name) { return
		 * name.matches(graphID + "_*"); }
		 * 
		 * });
		 * 
		 * for(File file : files){ if(!file.delete()){
		 * Logger_Java.info("Cannot delete file : " + file.getName()); result =
		 * "-1"; } }
		 */

		// Also we need to update the local catalog after deleting the graph
		// partition.
		// TODO in future : There will be a contention issue when multiple
		// threads try to write to the same file.
	...
	public void loadLocalStore(String graphID, String partitionID) {
		...
		AcaciaHashMapLocalStore graphDB = new AcaciaHashMapLocalStore(
				Integer.parseInt(graphID), Integer.parseInt(partitionID));
		graphDB.loadGraph();// We need to load the graph explicitly
		graphDBMap.put(gid, graphDB);
		loadedGraphs.add(gid);

		try {
			Logger_Java.info("Loaded the graph " + gid + " at "
					+ java.net.InetAddress.getLocalHost().getHostName());
		} catch (UnknownHostException ex) {
			Logger_Java.error("Error : " + ex.getMessage());
		}
		System.out
				.println("------------ Done Running from AAAAAAAAAAAAAAAAAAAAAAAAAAAA--------");
	}

	/**
	 * 28/10/2014 We need not to worry about which partition we are dealing with
	 * when loading and unloading graphs. It is because we know each place is
	 * handling only one subgraph from each graph. So we can directly refer the
	 * data by graph id without use of partition id.
	 * 
	 * @param graphID
	 */
	public void unloadLocalStore(String graphID, String partitionID) {
		...
		while (it.hasNext()) {
			Map.Entry<String, AcaciaHashMapLocalStore> pairs = (Map.Entry<String, AcaciaHashMapLocalStore>) it
					.next();
			if (pairs.getKey().equals(gid)) {
				((AcaciaHashMapLocalStore) pairs.getValue()).shutdown();
				try {
					Logger_Java
							.info("Unloaded the graph "
									+ gid
									+ " at "
	...
	public void insertEdge(long graphID, long startVertexID, long endVertexID) {
		...
		if (defaultGraph == null) {
			...
			if (graphDB == null) {
				Logger_Java
						.error("Error : The graph database instance is NULL.");
			}
		} else {
			graphDB = defaultGraph;
		}
		...
		System.out.println("Done adding edge : " + startVertexID + " "
				+ endVertexID);
	}

	/**
	 * This method returns the vertex's unique id if one exists. If not returns
	 * -1.
	 * 
	 * @param graphID
	 * @param vertexID
	 ...
	 */
	/*
	 * public long vertexExists(long graphID, long vertexID){ String resultStr =
	 * "-1"; GraphDatabaseService graphDB = null;
	 * 
	 * if (defaultGraph == null){ graphDB = graphDBMap.get(graphID); if(graphDB
	 * == null){ return -1; } }else{ graphDB = defaultGraph; }
	 * 
	 * ExecutionEngine engine = new ExecutionEngine(graphDB); ExecutionResult
	 * exResult = engine.execute("start n=node(*) where n.vid='" + vertexID +
	 * "' return n;"); Iterator itr = exResult.iterator();
	 * 
	 * if(itr.hasNext()){ resultStr = itr.next().toString(); Pattern p =
	 * Pattern.compile("\\d+"); Matcher m = p.matcher(resultStr);
	 * 
	 * while(m.find()){ resultStr = m.group(); break; } }
	 * 
	 * if (!resultStr.trim().equals("")){ return Long.parseLong(resultStr);
	 * }else{ return -1; } }
	 */

	/*
	 * public long deleteAllverticesandEdgesofGraph(String graphID){ long vcnt =
	 * 0; String resultStr = ""; GraphDatabaseService graphDB = null;
	 * 
	 * if (defaultGraph == null){ graphDB =
	 * graphDBMap.get(Integer.parseInt(graphID)); if(graphDB == null){ return
	 * -1; } }else{ graphDB = defaultGraph; }
	 * 
	 * ExecutionEngine engine = new ExecutionEngine(graphDB); ExecutionResult
	 * exResult = engine.execute("start n=node(*) return collect(id(n));");
	 * Iterator itr = exResult.iterator();
	 * 
	 * if(itr.hasNext()){ resultStr = itr.next().toString(); // //
	 * System.out.println("1111============================"); // Pattern p =
	 * Pattern.compile("\\d+(,[ ]\\d+)*"); //
	 * System.out.println("2222============================"); //
	 * System.out.println("resultStr : |" + resultStr+"|"); // Matcher m =
	 * p.matcher(resultStr); //
	 * System.out.println("wwwwww============================"); //
	 * while(m.find()){ // resultStr = m.group(); // break; // } //
	 * System.out.println("llllll============================"); // int beginIdx
	 * = resultStr.indexOf("["); int endIdx = resultStr.indexOf("]"); resultStr
	 * = resultStr.substring(beginIdx+1, endIdx); }
	 * 
	 * 
	 * if(!resultStr.contains("collect")){ String[] verts=resultStr.split(",");
	 * int len= verts.length;
	 * 
	 * for(int c=0; c < len; c++){ //This query returns the number of vertices
	 * that are affected(deleted). String vert = verts[c].trim(); if
	 * (!vert.equals("")){ ExecutionResult exResult2 =
	 * engine.execute("start n=node(" + vert + ") match n-[r]-() delete n,r;");
	 * Iterator itr2 = exResult.iterator();
	 * 
	 * if(itr2.hasNext()){ resultStr = itr2.next().toString();
	 * 
	 * Pattern p = Pattern.compile("\\d+"); Matcher m = p.matcher(resultStr);
	 * 
	 * while(m.find()){ vcnt += Long.parseLong(m.group()); break; } } } } }
	 * 
	...
	public String countVertices(String graphID, String partitionID) {
		...
		// result = "" + (Long.parseLong(result) - 1); //By default there is
		// avertex created by every Neo4j instance. Therefore we need to deduct
		// one to compensate this.
	...
	public String outDegreeDistribution(String graphID, String partitionID) {
		org.acacia.log.java.Logger_Java
				.info("Out degree distribution calculation started at : "
						+ org.acacia.util.java.Utils_Java.getHostName());
		...
		// engine.execute("start n=node(*) match n-->m return n, n.vid, count(m)");
		//
		...
		// String dataFolder =
		// Utils_Java.getAcaciaProperty("org.acacia.server.instance.datafolder");
		// String line = null;
		...
		int partitionId = -1;
		// try{
		// BufferedReader reader = new BufferedReader(new FileReader(dataFolder
		// + "/catalog"));
		//
		// while((line = reader.readLine()) != null){
		...
		// partitionId =
		// Utils_Java.getPartitionIDFromCatalog(Integer.parseInt(graphID));

		// System.out.println("partition ID : " + partitionId);
		// Next we request and get all the local to world degree distribution.
		// This will be a list of vertices of this
		// graph partition which are connected with the extrenal world, each
		// vertex will have its associated out degree.

		// AcaciaInstanceToManagerAPI
		// String serverHost, int graphID, int partitionID
		String serverHostName = Utils_Java.getServerHost();

		// Next, we need to resolve the out links that are from the vertices of
		// this subgraph to the external world
		HashMap<String, String> res = AcaciaInstanceToManagerAPI
				.getLambdaOutDegreeDistribution(serverHostName, graphID,
						partitionID);
		long vertex = 0;
		long outDegree = 0;
		long updatecount = 0;
		...
		System.out
				.println("=========================> updated this many vertices : "
						+ updatecount);
		...
		org.acacia.log.java.Logger_Java
				.info("Out degree distribution calculation completed at : "
						+ org.acacia.util.java.Utils_Java.getHostName());

		return resultSB.toString();
	}

	public String pageRankLocal(String graphID, String partitionID,
			String hostList) {
		String result = null;
		AcaciaHashMapLocalStore graphDB = null;
		...
		System.out.println("Started Approx Rank");
		result = ApproxiRank.run(graphID, partitionID, graphDB, hostList,
				serverHostName, -1); // Here we send -1 meaning we want the
										// entire pagerank vector, nit top-k
										// pageranks.
		System.out.println("Done Approx Rank");
		return result;
	}
