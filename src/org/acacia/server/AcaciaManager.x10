...
public class AcaciaManager {
	 ...
	 */
 	...
 	public static def countVertices(host:String, graphID:String, partitionID:String):long{
		 ...
		 public static def countVertices(graphID:String):long{
		 	val result:long = Long.parseLong((org.acacia.metadata.db.java.MetaDataDBInterface.runSelect("SELECT VERTEXCOUNt FROM ACACIA_META.GRAPH WHERE IDGRAPH=" + graphID).value as Rail[String])(0n));
		 	return result;
		 }
	 ...
	 public static def countEdges(graphID:String):long{
	 	val result:long = Long.parseLong((org.acacia.metadata.db.java.MetaDataDBInterface.runSelect("SELECT EDGECOUNT FROM ACACIA_META.GRAPH WHERE IDGRAPH=" + graphID).value as Rail[String])(0n));
	 	return result;		
	 }
	...
	public static def countGlobalTraingles(graphID:String):long{
		 ...
		 for(var i:Int = 0n; i < centralPartionCount; i++){
		 	...
		 	val store:AcaciaLocalStore = AcaciaLocalStoreFactory.load(Int.parse(graphID), i, Utils_Java.getAcaciaProperty("org.acacia.server.instance.datafolder") + File.separator + graphID + "_centralstore", true);
		 	store.loadGraph();
		 ...
		 while(itr1.hasNext()){
		 	...
		 	while(itr2.hasNext()){
		 		...
		 		if(uList != null){ //Because in local subgraph map we may mark only u -> v, but v may not have corresponding record in the map (i.e., v -> u).
		 			itr3:Iterator[Long] = uList.iterator() as Iterator[Long];
		 			while(itr3.hasNext()){
		 				u:Long = itr3.next();
		 				 //second for loop
		 				...
		 				if(nuList != null){
		 					itr4:Iterator[Long] = nuList.iterator() as Iterator[Long];
		 					while(itr4.hasNext()){
		 						nu:Long = itr4.next();
		 						 //Third for loop
