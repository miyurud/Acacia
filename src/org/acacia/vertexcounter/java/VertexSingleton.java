...
public class VertexSingleton {
    ...
    public void load() {
        synchronized (this) {
            ...
            String[] result = MetaDataDBInterface
                    .runSelectPureJava("SELECT idgraph,vertexcount FROM ACACIA_META.GRAPH");
            ...
            for (int i = 0; i < result.length; i += 2) {
                if (result[i + 1] == null) {
                    ...
                    graphMap.put(Long.parseLong(result[i]),
                            Long.parseLong(result[i + 1]));
                }
            }
        }
         ...
         * String value = null;
         * 
         * if(result.size > 0){ if((value=itr.next()) == null){
         * graphMap.put(Long.parse(result(0)), 0); }else{ System.out.println(""
         * + result(0) + " " + result(1)); graphMap.put(Long.parse(result(0)),
         * Long.parse(result(1))); } }
         */
    ...
    public boolean save() {
        ...
        while (it.hasNext()) {
            ...
            if (pairs.getKey() == currentGraph) {
                res = MetaDataDBInterface
                        .runUpdate("UPDATE ACACIA_META.GRAPH SET VERTEXCOUNT="
                                + currentVertex + " WHERE IDGRAPH="
                                + pairs.getKey());
            } else {
                res = MetaDataDBInterface
                        .runUpdate("UPDATE ACACIA_META.GRAPH SET VERTEXCOUNT="
                                + pairs.getValue() + " WHERE IDGRAPH="
                                + pairs.getKey());
            }
